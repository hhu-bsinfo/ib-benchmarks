...
void set_qp_state_to_rts(ib_queue_pair *queue_pair) {
    ...
    attr.retry_cnt = 1; // Maximum amount of retries after not receiving an ACK from the remote queue pair
    attr.rnr_retry = 1; // Maximum amount of retries after receiving a NACK from the remote queue pair
