...
class Benchmarks {
    ...
    void messageSendBenchmark(Connection connection, long msgCount) {
        ...
        try {
            ...
            while(msgCount > 0) {
                ...
                if(batchSize < 10) {
                    pendingComps -= connection.pollCompletionQueue(VerbsWrapper.CqType.SEND_CQ);
                    continue;
                }
    ...
    void messageRecvBenchmark(Connection connection, long msgCount) {
        ...
        try {
            ...
            pendingComps -= connection.pollCompletionQueue(VerbsWrapper.CqType.RECV_CQ);
    ...
    void pingPongBenchmarkServer(Connection connection, long msgCount) {
        ...
        try {
            ...
            while(msgCount > 0) {
                ...
                do {
                    polled = connection.pollCompletionQueue(VerbsWrapper.CqType.SEND_CQ);
                } while(polled == 0);

                do {
                    polled = connection.pollCompletionQueue(VerbsWrapper.CqType.RECV_CQ);
                } while(polled == 0);
